...
async function standupOrg(org, afterStandup) {
  const testOrg = JSON.parse(JSON.stringify(testOrg1));

  testOrg.id_ref = getRandomString();

  const theOrg = await Organization.create(testOrg);
...
async function createEventCategory(category, orgId) {
  const testEventCategory = JSON.parse(JSON.stringify(category));
...
async function createEventRole(eventRole, eventId) {
  const testEventRole = JSON.parse(JSON.stringify(testEventRole1));
...
async function createEventPosition(eventPosition, eventId, eventRoleId) {
  const testEventPosition = JSON.parse(JSON.stringify(testEventPosition1));
...
it('fetches an org by ref', async () => {
  await standupOrg(testOrg1, async (org) => {
    const gottenOrg = await Organization.get({ id_ref: org.id_ref });
