...
import { Event, Person, PersonSystemRole } from './';
...
export interface Person extends PersonDataInsert {
	...
	person_system_role?:  PersonSystemRole.PersonSystemRole[]
}
...
export function makeAPIFriendly(person: Person): PersonUpdate {
	...
	if (person.person_system_role) {
		response.roles = person.person_system_role.map( personRole => personRole.system_role.role);
	}
...
/**
 * Helper method for manipulating dates
 */
...
/**
 * Gets a list of upcoming volunteer opportunities (i.e., events) for a given user's organizations
 */
export async function getUpcomingEvents(person: PersonIdentifier, days = 14) {
	
	const personEvents = await get({person, includeEvents: true});

	console.log("person before prettification");
	console.log(personEvents);

	console.log("person after prettification");
	console.log(makeAPIFriendly(personEvents));


	return personEvents.organization_person.map(orgPerson => {
		const org = orgPerson.organization;
		return {
			id_ref: org.id_ref,
			name: org.name,
			events: org.event.filter(event => 
				event.start_date > new Date() && event.start_date < addDays(new Date(), days)
